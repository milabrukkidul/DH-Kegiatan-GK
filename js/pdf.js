...
const PDFExport = (() => {
  ...
  function buildHTML({ kegiatan, hadir, setting }) {
    ...
    // Urutan presensi: Pengawas → Kepala → Wakabid → Guru → Operator → Karyawan → Guru Bantu
    ...
    // Statistik — hitung dengan Wakabid digabung
    const stat = {
      'Pengawas Madrasah': 0,
      'Kepala Madrasah':   0,
      'Wakabid':           0,
...
<div class="page">
  ...
  <div class="stat-row">
    ...
    <div class="stat-box">
      <div class="num" style="color:#4a148c">${stat['Pengawas Madrasah']}</div>
      <div class="lbl">Pengawas</div>
    </div>` : ''}
    ${stat['Kepala Madrasah'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#1a237e">${stat['Kepala Madrasah']}</div>
      <div class="lbl">Kepala</div>
    </div>` : ''}
    ${stat['Wakabid'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#01579b">${stat['Wakabid']}</div>
      <div class="lbl">Wakabid</div>
    </div>` : ''}
    ${stat['Guru'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#1967d2">${stat['Guru']}</div>
      <div class="lbl">Guru</div>
    </div>` : ''}
    ${stat['Operator'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#e65100">${stat['Operator']}</div>
      <div class="lbl">Operator</div>
    </div>` : ''}
    ${stat['Karyawan'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#2e7d32">${stat['Karyawan']}</div>
      <div class="lbl">Karyawan</div>
    </div>` : ''}
    ${stat['Guru Bantu'] > 0 ? `
    <div class="stat-box">
      <div class="num" style="color:#7b1fa2">${stat['Guru Bantu']}</div>
      <div class="lbl">Guru Bantu</div>
    </div>` : ''}
  </div>
