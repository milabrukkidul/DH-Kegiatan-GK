...
async function doLihatDetail(id) {
  ...
  try {
    ...
    document.getElementById('detail-stats').innerHTML = `
      ...
      ${stat['Pengawas Madrasah'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:#4a148c">${stat['Pengawas Madrasah']}</div>
        <div class="stat-lbl">Pengawas</div>
      </div>` : ''}
      ${stat['Kepala Madrasah'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:#1a237e">${stat['Kepala Madrasah']}</div>
        <div class="stat-lbl">Kepala</div>
      </div>` : ''}
      ${stat['Wakabid'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:#01579b">${stat['Wakabid']}</div>
        <div class="stat-lbl">Wakabid</div>
      </div>` : ''}
      ${stat['Guru'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:#1967d2">${stat['Guru']}</div>
        <div class="stat-lbl">Guru</div>
      </div>` : ''}
      ${stat['Operator'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:#e65100">${stat['Operator']}</div>
        <div class="stat-lbl">Operator</div>
      </div>` : ''}
      ${stat['Karyawan'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:var(--clr-green-dk)">${stat['Karyawan']}</div>
        <div class="stat-lbl">Karyawan</div>
      </div>` : ''}
      ${stat['Guru Bantu'] > 0 ? `
      <div class="stat-card">
        <div class="stat-num" style="color:var(--clr-purple)">${stat['Guru Bantu']}</div>
        <div class="stat-lbl">Guru Bantu</div>
      </div>` : ''}`;
